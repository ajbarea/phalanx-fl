...
- [ ] **Surface termination events in experiment output** — **JSON + server-side plot slices shipped 2026-05-28** (see Recently shipped): early termination writes `out/<run>/termination.json` (`terminated_early`/`termination_round`/`termination_reason`/`termination_policy`), and the matplotlib accuracy/loss plots (`show_plots_within_strategy`) now draw a dotted vertical marker at the termination round with the reason. **Remaining:** thread the same summary into the snapshot HTML report (run-level header, not the per-client/round attack snapshots — see scoping note below). Trigger: UX must make removal failure visible.
- [ ] **Frontend (React) alert + plot marker for early termination** — surface a toast/alert in the React UI when a run terminates early, and replicate the termination marker on the *interactive* (frontend) accuracy/loss plots. The server-side matplotlib marker shipped 2026-05-28; this is the React half. To wire it, `save_plot_data_json` would need to emit the `termination.json` payload alongside the metric histories (deliberately deferred — YAGNI until this frontend work is picked up, so the JSON contract is added with its consumer, not speculatively). Trigger: researchers should see "removal gone too far" at a glance, not as a status-code surprise.
