...
- **2026-05-25** — **Dependabot dependency automation** rewritten to the canonical fleet shape (techne `templates/dependabot.yml.example`): uv + npm (`/frontend`) + docker (`/` + `/frontend`) + github-actions, minor/patch grouped one-PR-per-ecosystem. Migrated the legacy `pip` ecosystem → native `uv` (GA 2025-03-13) and added the docker coverage the prior config lacked. research(2026-05): Dependabot over Renovate (native, no Mend-app gate).
...
- **2026-05-23** — **Per-attack specialized visuals in composites** [#39]. Composite runs emit each attack's `*_visual.png` alongside `composite_synopsis.png` via `extract_individual_attack_types` + per-type dispatch in `save_visual_snapshot`. Singles preserve legacy `{attack_type}_visual.png` naming.
- **2026-05-23** — **Dataset System Rework Phase 2E** [#34, #36]. `image_dataset_loader.py` + `huggingface_image_dataset_loader.py` retired; `config/test_hf_datasets.py` rewired to `FederatedDatasetLoader`. Closes the migration arc.
- **2026-05-23** — **Modular Partitioning System Phase 0**. `partitioner_registry.py` adds 10 new partitioner wirings (linear / exponential / square / shard / continuous / grouped_natural_id / size / inner_dirichlet / distribution) on top of the existing iid / dirichlet / pathological / natural_id quartet. `FederatedDatasetLoader._create_partitioner` collapsed if/elif → 7-line registry delegate. Phase 1 (config + docs) + Phase 2 (frontend dropdown) deferred until first concrete UX surfacing.
...
- **2026-05-22** — **File-consolidation pass closed out**. `simulation_strategies/` + `dataset_loaders/` use registry+factory shape; `*_handlers/` + `client_models/` audited as N/A.
...
- **2026-04-06** — **Transformer model merge**. `bert_model_definition.py` + `text_classifier_model.py` → `transformer_models.py`; unified `load_hf_model(model_name, task, ...)` with `mlm` / `seq_cls` task dispatch.
