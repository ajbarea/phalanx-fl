...
- [ ] **1B (cifar10 / cinic10 wiring):** JSON entries shipped in 1A but `image_transformer` is null and they're not in `LOADER_REGISTRY`. Phase 2A registers `cifar10_image_transformer` (CIFAR-10-specific normalize stats), adds entries to `LOADER_REGISTRY` mapping to `_build_cifar100`-equivalent builder.
