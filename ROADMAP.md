...
- [ ] **(re-scoped) Runtime floor for Krum-family removal** — `krum_based_removal` / `multi_krum_based_removal` compose Krum (n ≥ 2f+3) but do **not** construct a `TerminationHandler`; config-time validation only checks the *starting* roster, so round-by-round removal could in principle drop the live federation below 2f+3 and corrupt the Krum selection. Audit whether removal can actually breach the floor mid-run (the removal cap may already prevent it) and, if so, add a runtime guard. This is the genuine "removal breaks aggregation → silent NaN" risk, narrowed to the strategies it applies to. Trigger: math constraints reached should stop, not corrupt.
