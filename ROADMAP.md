...
- [x] **`CITATION.bib` generation per output run — already shipped (found 2026-06-01).** `DirectoryHandler._save_citation_bib` writes a per-run `CITATION.bib` alongside `MANIFEST.json` (verified present in `out/<run>/`). **Open follow-up (AJ's call — attribution):** the generated entry still reads `author = {IntelliFL Execution Framework}` + `howpublished = {…dmitrykoro/fl-execution-framework}` (the pre-de-fork upstream) — stale against the Phalanx rebrand, but how the spinoff credits the origin vs itself is a research-identity decision, left for AJ. research(2026-06): the 2026 software-citation standard is **`CITATION.cff`** (YAML, GitHub-native "Cite this repository" + auto-BibTeX export, software-specific entity types) — consider a repo-level `CITATION.cff` over the hand-rolled per-run `.bib` once attribution is settled.
