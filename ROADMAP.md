...
- [ ] **Surface termination events in experiment output** — **JSON slice shipped 2026-05-28** (see Recently shipped): early termination now writes `out/<run>/termination.json` (`terminated_early`/`termination_round`/`termination_reason`/`termination_policy`) so the researcher sees "terminated round 7: GRACEFUL hit 0 clients" without grepping logs. **Remaining:** thread that summary into the snapshot HTML report and annotate the accuracy/loss plots with the termination round + reason (the plot annotation overlaps the frontend item below). Trigger: UX must make removal failure visible.
