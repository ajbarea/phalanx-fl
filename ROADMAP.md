...
- **2026-05-23** — Deterministic label-flip permutation per client [PR queued]. `apply_label_flipping` (`intellifl/attack_utils/poisoning.py:50`) was calling `torch.randperm(num_classes)` against the global RNG, so a single malicious client drew a fresh permutation every batch — the model treated it as random label noise and averaged it out. Fix threads `client_id` through `apply_poisoning_attack → _dispatch_label_flipping → apply_label_flipping`; when set, the permutation draws from `torch.Generator().manual_seed(42 + client_id)` (matching the `mock_datasets.py:88` convention and BlazeFL's isolated-RNG-per-client pattern — research(2026-05): arXiv:2604.03606). All 4 flower_client.py call sites (CNN train/eval + transformer train/eval) updated to pass `client_id=self.client_id`. Other dispatch fns accept and ignore the kwarg so the registry dispatch stays uniform — future per-client RNG needs (gaussian noise jitter, etc.) can use the same surface without a second signature change. Coverage: 4 new label-flip determinism tests + 1 `apply_poisoning_attack` integration test. Now `attack_schedule`-based experiments produce the expected divergence from clean baselines.
- **2026-05-23** — Dataset System Rework Phase 2E (first tier) [#34]. Deletes the orphaned `intellifl/dataset_loaders/image_dataset_loader.py`, the now-empty `_LOCAL_CNN_KEYWORDS` tuple, the `_build_cnn` builder, and the dead `ImageDatasetLoader` mock-patches across four test files. The 467-line `tests/unit/test_dataset_loaders/test_image_dataset_loader.py` removed wholesale. Net diff: -617/+10. All 79 affected tests + whole-repo lint green. Second tier (`huggingface_image_dataset_loader.py` deletion) deferred behind a `config/test_hf_datasets.py` retire-or-rewire decision and tracked separately.
...
- **2026-05-23** — Dataset System Rework Phase 2B (FEMNIST family migration — closes Phase 2B). `femnist_iid` + `femnist_niid` migrated from local `ImageDatasetLoader` to `FederatedDatasetLoader` reading `flwrlabs/femnist`. `_build_federated_dataset_loader` gains a third precedence tier — config > **JSON per-dataset default** > Dirichlet alpha=0.5 fallback — so the JSON entries can declare `partitioning_strategy="iid"` (femnist_iid) and `partitioning_strategy="natural_id"` + `partition_by="writer_id"` (femnist_niid) without forcing every caller to set them on `StrategyConfig`. `_CNN_REGISTRY["femnist_iid"]["num_classes"]` bumped 10 → 62 to match canonical LEAF FEMNIST (research(2026-05): the 10-class digit subset was AJ-side preprocessing of the local tarball, not in any community baseline). `_LOCAL_CNN_KEYWORDS` is now an empty tuple — Phase 2E will delete it along with `_build_cnn` and `ImageDatasetLoader`. Schema test split: family-wide `_HF_MEDMNIST_CNN_KEYWORDS` assertion checks transformer + `_CNN_REGISTRY` membership; two new per-family tests cover the MedMNIST canonical mirror + FEMNIST partitioning declarations.
