...
**Preserve (lineage / origin — never rebrand):** `docs/research/rit-capstone-2025-intellifl/`, the capstone nav
label in `zensical.toml`, and the "IntelliFL predecessor" prose in `docs/research/index.md` document the project's
*origin* as the RIT IntelliFL/InteFL capstone — factual history, kept. The de-fork story is "Phalanx grew out of
the InteFL capstone," not "InteFL never existed."

**Citation — cite our own (AJ, reversed 2026-06-01).** The earlier (2026-05-27) decision to credit `IntelliFL
Execution Framework` + `dmitrykoro/fl-execution-framework` in the generated `CITATION.bib` is **reversed** — cite
Phalanx itself. `_save_citation_bib` now emits `author = {Barea, AJ}` + `github.com/ajbarea/phalanx-fl`, and a
repo-level `CITATION.cff` (CFF 1.2.0, GitHub-native) cites Phalanx / AJ Barea. Software citation cites the software
artifact directly, so no separate paper is needed (the prior "no Phalanx paper → credit IntelliFL" reasoning
conflated software- with paper-citation). The lineage/origin docs above are untouched — recording history is
distinct from how the software is cited. Open: confirm the author-name form (formal "Arnaldo Barea" + ORCID?) on
the CFF.
...
- [x] **Per-run `CITATION.bib` + repo-level `CITATION.cff` — cite our own (resolved 2026-06-01).** `_save_citation_bib` writes a per-run `CITATION.bib`; both it and a new repo-level `CITATION.cff` (CFF 1.2.0, GitHub-native "Cite this repository") now cite **Phalanx / AJ Barea / github.com/ajbarea/phalanx-fl** — the pre-de-fork `IntelliFL Execution Framework` + `dmitrykoro/fl-execution-framework` attribution was removed (AJ: "ditch dmitry, cite our own"). Guarded by `tests/unit/test_citation.py` (valid CFF + no upstream regression). Open: confirm the author-name form (formal "Arnaldo Barea"? + ORCID) on the CFF. research(2026-06): CFF 1.2.0 is the software-citation standard — GitHub/Zenodo/Zotero render it with auto-BibTeX export.
- [ ] **Output directory IEEE-artifact restructure** — `out/<run>/` is currently flat (config + CSVs + PDFs + logs + `MANIFEST.json` all at root). Restructure into `config/`, `data/{metrics,attack_analysis}/`, `visualizations/{accuracy,loss,removal,analysis}/`, `logs/`, `latex/`, `reports/`; keep `MANIFEST.json` + `status.json` + (planned) `CITATION.bib` + `README.md` at run root. Add per-run `README.md` with quick stats, key findings, file-organization map, and reproducibility metadata (git commit, system info, config checksum). Trigger: aligns with FAIR + IEEE artifact-evaluation badge criteria ("Available", "Reusable"); pairs with the `CITATION.bib` item above and the existing `MANIFEST.json` infra. **Per-run `README.md` shipped 2026-06-01** (`DirectoryHandler._save_run_readme`: config summary + file-organization map + pointers to `MANIFEST.json`/`CITATION.bib` for reproducibility metadata). The directory *restructure* itself remains, deferred as higher-risk — it changes paths the frontend + plot consumers read, so it needs consumer updates in the same slice.
