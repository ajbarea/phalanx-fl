...
def test_run_task_sets_utf8_defaults(monkeypatch) -> None:
    ...
    def fake_streamed_run(command, *, check, label, cwd, env):  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(dev_cli, "streamed_run", fake_streamed_run)

    assert dev_cli.run_task("check-env") == 0
    assert captured["cwd"] == dev_cli.PROJECT_ROOT
    assert captured["check"] is False
    assert captured["label"] == "check-env"
...
def test_yolo_runs_clean_setup_then_upgrade(monkeypatch) -> None:
    ...
    def fake_run_process(command: list[str], *, cwd: Path, label: str | None = None) -> int:
